...
def generate_positive(rows=400000, file='../../../reference.npy', word=1, length=200):
    data = np.load(file)
    ...
    ts = int(0.2*rows)
    train_set_X = X[:tr]
    val_set_X = X[tr:(tr+val)]
    # print("validation split", val_set_X.shape)
    test_set_X = X[(tr+val):]
    train_set_Y = Y[:tr]
    val_set_Y = Y[tr:(tr+val)]
    test_set_Y = Y[(tr+val):]
...
def load_data_set(type,max_len,vocab_size,batch_size):
    ...
    if type == 0:
        ...
        word_to_id['g'] = 2
        word_to_id['t'] = 3
        X_train,Y_train, X_val, Y_val = generate_positive(5000000, length=max_len)
        # print(X_val.shape)
