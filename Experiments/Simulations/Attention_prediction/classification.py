...
if classification_type == 'dna':
    ...
    params_set["attention_hops"] = 10
    params_set["use_embeddings"] = False
    if params_set["use_embeddings"]:
        embeddings = load_glove_embeddings("glove/glove.6B.50d.txt",word_to_id,50)
    else:
        embeddings = None
    attention_model = StructuredSelfAttention(batch_size=train_loader.batch_size,lstm_hid_dim=model_params['lstm_hidden_dimension'],d_a = model_params["d_a"],r=params_set["attention_hops"],emb_dim=emb_dim, vocab_size=len(word_to_id),max_len=MAXLENGTH,type=1,n_classes=4,use_pretrained_embeddings=params_set["use_embeddings"],embeddings=embeddings)
