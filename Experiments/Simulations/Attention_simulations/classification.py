...
from attention.train import train,get_activation_wts,evaluate,validate
...
def binary_classfication(attention_model,train_loader,val_loader=None, epochs=5,use_regularization=True,C=1.0,clip=True):
    ...
    optimizer = torch.optim.RMSprop(attention_model.parameters(), lr=5e-3)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=20000, gamma=0.7)
    for i in range(20):
        print("Epoch", i+1)
        scheduler.step()
        train(attention_model,train_loader,loss,optimizer,scheduler,1,use_regularization,C,clip, steps=10000)
        # torch.save(attention_model.state_dict(), 'model')
        if val_loader is not None:
            validate(attention_model, val_loader, loss, use_regularization, C=1.0, steps=1000)
...
if classification_type == 'dna':
    ...
    train_loader,val_loader,x_test_pad,word_to_id = load_data_set('dna',MAXLENGTH,model_params["vocab_size"],model_params['batch_size']) #load the reuters dataset
    #Using pretrained embeddings
...
if classification_type == 'iid':
    model_params['batch_size'] = 30
    model_params["vocab_size"] = 2
    n_classes = 1
    emb_dim = 10

    train_loader,val_loader,x_test_pad,word_to_id = load_data_set('binary',MAXLENGTH,model_params["vocab_size"],model_params['batch_size']) #load the reuters dataset
    #Using pretrained embeddings
    print(len(word_to_id))
    params_set["attention_hops"] = 20
    params_set["use_embeddings"] = False
    ...
    if params_set["use_embeddings"]:
        ...
        embeddings = None
    attention_model = StructuredSelfAttention(batch_size=train_loader.batch_size,lstm_hid_dim=model_params['lstm_hidden_dimension'],d_a = model_params["d_a"],r=params_set["attention_hops"],emb_dim=emb_dim, vocab_size=len(word_to_id),max_len=MAXLENGTH,type=0,n_classes=n_classes,use_pretrained_embeddings=params_set["use_embeddings"],embeddings=embeddings)
 
    #Using regularization and gradient clipping at 0.5 (currently unparameterized)
    attention_model.cuda()
    binary_classfication(attention_model,train_loader,val_loader,epochs=params_set["epochs"],use_regularization=params_set["use_regularization"],C=params_set["C"],clip=params_set["clip"])
    classified=True
    #wts = get_activation_wts(multiclass_attention_model,Variable(torch.from_numpy(x_test_pad[:]).type(torch.LongTensor)))
    #print("Attention weights for the data in multiclass classification are:",wts)
