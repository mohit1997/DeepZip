...
def train(attention_model,train_loader,criterion,optimizer, scheduler, epochs = 5,use_regularization = False,C=0,clip=False, steps=None):
    """
        Training code
    ...
    for i in range(epochs):
        ...
        for batch_idx,train in enumerate(train_loader):
            ...
            if not bool(attention_model.type) :
                ...
                correct+=torch.eq(torch.round(y_pred.type(torch.DoubleTensor).cuda().squeeze(1) + 1e-8),y.type(torch.DoubleTensor).cuda()).sum().item()
                if use_regularization:
                    try:
                    ...
                    loss = criterion(y_pred.type(torch.DoubleTensor).cuda().squeeze(1) + 1e-8,y)
            ...
            # print((n_batches + 1)*x.size(0))
            if (n_batches + 1) % (200) == 0:
                print("Step", n_batches, "nll_loss is", nllloss/n_batches, "current batch loss", loss1.item())
...
def validate(attention_model, val_loader, criterion, use_regularization=False, C=0, steps=None):
    ...
    with torch.no_grad():
        for batch_idx,train in enumerate(val_loader):
            ...
            if attention_model.type == 0:
                ...
                loss = criterion(y_pred + 1e-8,y.type(torch.FloatTensor).cuda())
            else:    
                correct+=torch.eq(torch.max(y_pred.type(torch.LongTensor).cuda(),1)[1],y.type(torch.LongTensor).cuda()).sum().item()
                loss = criterion(y_pred,y.type(torch.LongTensor).cuda())
            n_batches += 1
            if(n_batches + 1)%100 == 0:
                print("Validation avg_loss till now is",total_loss/n_batches)
            if steps is not None:
                if n_batches == steps:
                    break
            # print(n_batches)
...
def predict(attention_model, val_loader, criterion, use_regularization=False, C=0):
    attention_model.eval()
    losses = []
    accuracy = []
    total_loss = 0
    ...
    with torch.no_grad():
        for batch_idx,train in enumerate(val_loader):
            ...
            y_pred,att = attention_model(x)
            # print(y_pred.type())
            pred_out.append(y_pred.detach().cpu().numpy())

            # print(y_pred.detach().cpu().numpy().dtype)
