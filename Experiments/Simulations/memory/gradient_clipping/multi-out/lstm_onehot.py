...
def fit_lstm(X, Y, bs, nb_epoch, neurons):
	...
	model.add(CuDNNLSTM(32, batch_input_shape=(bs, X.shape[1], X.shape[2]), stateful=True, return_sequences=True))
...
series = np.load('markov_seq.npy')[:1000000]
...
for r in range(1):
	# fit the model
	print("Hello")
	lstm_model = fit_lstm(X_onehot, Y_onehot, batch_size, nb_epoch=5, neurons=32)
