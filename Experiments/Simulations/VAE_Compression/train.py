    ...
    for epoch in range(args.epochs):
        ...
        for split in splits:
            ...
            for iteration, batch in enumerate(data_loader):
                ...
                # loss calculation
                NLL_loss, KL_loss, KL_weight, acc = loss_fn(logp, batch['target'],
                ...
                if tracker['ELBO'].size() == torch.Size([0]):
                    a = tensor(np.array([loss.item()]))
                    tracker['ELBO'] = a
                else:    
                    tracker['ELBO'] = torch.cat((tracker['ELBO'], tensor(np.array([loss.item()]))))
                ...
                if iteration % args.print_every == 0 or iteration+1 == len(data_loader):
                    print("%s Batch %04d/%i, Loss %9.4f, NLL-Loss %9.4f, KL-Loss %9.4f, acc %6.8f, KL-Weight %6.3f"
                        %(split.upper(), iteration, len(data_loader)-1, loss.item(), NLL_loss.item()/batch_size, KL_loss.item()/batch_size, acc, KL_weight))
