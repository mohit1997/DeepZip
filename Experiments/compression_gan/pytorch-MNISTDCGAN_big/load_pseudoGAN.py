...
class seed_generator(nn.Module):
    def __init__(self, d=16):
        ...
        self.conv1 = nn.Conv2d(d, d*16, 2, 1, 0)
        self.conv1_bn = nn.BatchNorm2d(d*16)
        self.conv2 = nn.Conv2d(d*16, d*8, 2, 1, 0)
        self.conv2_bn = nn.BatchNorm2d(d*8)
        self.conv3 = nn.Conv2d(d*8, channels, 2, 1, 0)
    # weight_init
...
def show_compressed(seed, num_epoch, path='result.png'):
    z_ = seed
    with torch.no_grad():
        z_ = Variable(z_.cuda())
...
seed_z_ = torch.randn((grid_size*grid_size, 16*4*4)).view(-1, 16, 4, 4)
...
for epoch in range(train_epoch):
    ...
    for x_, _ in train_loader:
        ...
        y_real_ = torch.zeros(grid_size*grid_size)
        y_real_ = Variable(y_real_.cuda())

        S_result = S(z_)
        G_result = G(S_result)
        S_train_loss = MSE_loss(G_result, Variable(torch.from_numpy(np.repeat(target_image.numpy(), grid_size*grid_size, 0)).cuda())) 
    ...
    show_compressed(seed_z_, (epoch+1), path=comp_p)
