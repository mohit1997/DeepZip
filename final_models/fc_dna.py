...
parser.add_argument('-gpu', action='store', default="1",
                    dest='gpu',
                    help='choose gpu number')
parser.add_argument('-name', action='store', default="model_chr20",
                    dest='name',
...
def fit_lstm(X, Y, bs, nb_epoch, neurons):
	...
	decay = bs*1.0/len(X)
	model = Sequential()
	model.add(Embedding(y.shape[1], 5, batch_input_shape=(bs, X.shape[1])))
	# model.add(CuDNNLSTM(128, stateful=False, return_sequences=True))
	# model.add(CuDNNLSTM(32, stateful=False, return_sequences=False))
